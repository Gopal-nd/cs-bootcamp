    ...
    <div className="">
      <div className="relative h-screen w-full bg-neutral-200 md:pt-8 ">
        <div className='flex items-center justify-between max-w-4xl mx-auto px-2 py-2 md:rounded-full  bg-white border border-neutral-200 shadow-nd'>
          ...
          </button> {open && (<div className=" flex md:hidden absolute flex-col max-w-[98%] mx-auto inset-0-x top-20 w-full bg-white items-start gap-4 p-4 text-neutral-500 shadow-nd  rounded-lg">
            {links.map((link) => (
              <Link
                key={link.name}
                href={link.href}
      ...
      <h3 className="text-2xl ">Hello what is the best thing to do know </h3>
    </div>
  )
}
